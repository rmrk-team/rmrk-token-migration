...
import { bytecode } from '../artifacts/contracts/tokenAttributes/RMRKTokenAttributesRepositoryDraft.sol/RMRKTokenAttributesRepositoryDraft.json';
...
async function calculateSaltInRange(start: number, end: number, ethers: any) {
  ...
  // i used to generate the saltHex resulting in 7E57 is
  for (let i = start; i < end; i++) {
    const create2Addr = await create2Address(deployProxy.address, saltHex, bytecode, ethers);
    if (i % 100000 === 0) {
      console.log(`i is: ${i}`);
    }
    if (create2Addr.slice(2, 6) === '4778' && create2Addr.slice(40, 42) === '00') {
      console.log('Found!');
